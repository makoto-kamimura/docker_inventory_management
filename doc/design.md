...
| F-07  | Web UI          | 在庫一覧 / カテゴリ追加 / 物品追加 / 増減アイコンボタン (`＋`/`−`) / 履歴 Modal / 分析タブ (横棒グラフ) | 実装済 |
| F-08  | モバイル UI     | Web と同等の機能 + プルリフレッシュ + バーコードスキャン           | 実装済 |
...
- **セキュリティ**: アプリ層の認証/認可は未実装 (将来 Sanctum 等)。本番は Caddy で自動 TLS + 内部ネットワーク隔離、CORS は同一オリジン構成のため通常チェックされない
