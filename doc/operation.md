...
機能: ログイン / 在庫一覧 (在庫切れ絞り込み) / カテゴリ追加 / 物品追加 / 保管場所追加 / 在庫増 (＋1, **在庫0からの補充時は金額入力モーダル**) / 在庫減 (−1) / カテゴリ移動 / 履歴 Modal (金額・更新者表示) / プルリフレッシュ / ログアウト / **バーコードスキャン** (一覧ヘッダの `⌖` ボタン。在庫0の物品をスキャンした場合も金額入力モーダルを経由)。トークンはメモリ保持のため**アプリ再起動で再ログインが必要**。
